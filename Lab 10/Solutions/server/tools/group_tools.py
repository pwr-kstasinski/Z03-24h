...
def get_group(group_id: int) -> Group:
    ...
    try:
        return session.query(Group).filter_by(id=group_id).one()
    except:
        return None
    finally:
        session.close()
...
def get_user_group(user_id: int) -> [Group]:
    session: Session = DB_Session()

    try:
        return session.query(group_members_table).filter_by(user_id=user_id).all()
    except:
        return []
    finally:
        session.close()
